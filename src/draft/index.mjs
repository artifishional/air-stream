...
some()
  .setup([], (datas, streams) => {
    // Если схема потоков была изменена,
    // то рассчитанное состояние не будет иметь значения
    // метод трансформации будет вызван в новой конфигурации
    return { streams: { 3: true } };
  });

/**
 * к любому потоку можно подключиться в процессе
 * поэтому приходится кэшировать результат выполнения
 * последних операций, чтобы дублировать его для дочерних узлов
 */

export default ({ obtain }) => obtain('@a')
  .someone(obtain('@b'))
  .someone(obtain('@c'))
  .dyn();
...
// AA.add(d);
/**
 * - Как оптимизировать работу кэша завершенных операций
 *  (для узлов с асинхронными окончаниями)
 * - Как оптимизировать работу по поиску и преобразованию дерева
 * первоИсточинков (WSP)
 *     - состав WSPs может меняться от узла у узлу, а для
 *       комбинируемых узлов необходимо учитывать их количество
 *     - WSPs могут меняться на ходу (во время каждой операции)
 *
 *
 */
